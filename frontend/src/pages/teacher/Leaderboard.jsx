...
export default function TeacherLeaderboard() {
  ...
  const fetchLeaderboard = async () => {
    try {
      setLoading(true);
      const response = await api.get('/teacher/leaderboard', { params: { metric: filter, year } });
  ...
  if (loading && leaderboard.length === 0) return <LoadingSpinner fullscreen />;

  return (
    <div className="page-container">
      <PageHeader
        title="Department Leaderboard"
        subtitle="Top performers across the department"
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-[var(--text-secondary)]">
                  {user.class?.name || (user.year ? `Year ${user.year}` : '—')}
                </p>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-xl border" style={{ borderColor: 'var(--border)', background: 'var(--bg-base)' }}>
                <div className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Year</div>
                <div className="font-semibold" style={{ color: 'var(--text-primary)' }}>{selectedStudent.year || '—'}</div>
              </div>
              <div className="p-3 rounded-xl border" style={{ borderColor: 'var(--border)', background: 'var(--bg-base)' }}>
